// ==UserScript==
// @name         Cloudy Nights Collapsible Sidebar & Theme Toggle (Minified)
// @namespace    http://tampermonkey.net/
// @version      7.7
// @description  Optimized: Dark/Light themes selector, collapsible sidebar/header.
// @author       chvvkumar
...
[data-theme="dark-sky"] {
    ...
    --accent-primary: #E83C00; --accent-secondary: #FF8A65; --accent-primary-rgb: 232, 60, 0;
    --text-primary: #C0C0C0; --text-secondary: #B0B0B0; --text-muted: #888888;
    --border-color: #333333; --border-light: #444444;
    --shadow-sm: 0 1px 3px rgba(0,0,0,0.5); --shadow-md: 0 3px 6px rgba(0,0,0,0.8);
    --success: #10b981; --warning: #f59e0b; --error: #ef4444;
}

/* Dark Red Astronomy Theme */
[data-theme="dark-red-astronomy"] {
    --primary-bg: #000; --secondary-bg: #1A0000; --tertiary-bg: #1A0000;
    --accent-primary: #D32F2F; --accent-secondary: #CC0000; --accent-primary-rgb: 211, 47, 47;
    --text-primary: #B73E3E; --text-secondary: #8A3333; --text-muted: #442222;
...
/* COMPLETELY HIDE SIDEBAR WHEN COLLAPSED (Use Display None) */
.cn-sidebar-collapsed #ipsLayout_sidebar {
    width: 0 !important; min-width: 0 !important;
    margin: 0 !important; padding: 0 !important;
    display: none !important; /* Ensures it is removed from layout flow */
}

/* Ensure main area expands fully */
...
#cn-theme-selector-li button select {
    ...
    padding: 0; margin: 0; cursor: pointer; z-index: 2;
}
#cn-theme-selector-li button {
    ...
    const setupSidebar = () => {
        ...
        // ROBUST FINDING: Try multiple locations for the toolbar list
        let list = $('.ipsToolList_horizontal') ||
                   $('.ipsPageHeader .ipsToolList') ||
                   $('.ipsPageHeader + .ipsClearfix .ipsToolList');

        // FALLBACK: If no toolbar exists (e.g. on Search/Discovery pages), create one inside the header
        if (!list) {
            const header = $('.ipsPageHeader');
            if (header) {
                list = document.createElement('ul');
                list.className = 'ipsToolList ipsToolList_horizontal ipsPos_right';
                list.style.marginTop = '0'; // Ensure alignment
                // Append to header so it sits alongside existing title/controls
                header.appendChild(list);
            }
        }

        // If we still can't find/create a list, fallback to finding ANY toollist (rare case)
        if (!list) list = $('.ipsToolList');

        if (!list) return; // Exit only if absolutely no place to put buttons found
